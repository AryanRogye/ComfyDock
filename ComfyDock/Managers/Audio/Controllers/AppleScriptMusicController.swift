...
final class AppleScriptMusicController : NowPlayingController {
    ...
    /// "what is playing"
    private var is_spotify_playing = false
    private var is_apple_music_playing = false
    ...
    private func clearNowPlayingInfo(
        _ trackNameChanged        : @escaping (String) -> Void,
        _ artistNameChanged       : @escaping (String) -> Void,
        _ albumNameChanged        : @escaping (String) -> Void,
        _ artworkImageChanged     : @escaping (NSImage?) -> Void,
        ...
        _ musicProviderChanged    : @escaping (MusicProvider) -> Void
    ) {
    ...
    func getNowPlayingInfo(
        ...
        DispatchQueue.global(qos: .utility).async { [weak self] in
            ...
            self.is_spotify_playing = isSpotifyPlaying()
            self.is_apple_music_playing = isAppleMusicPlaying()
            ...
            if is_apple_music_playing {
                ...
                DispatchQueue.main.async {
                    if let info = self.getMusicInfo() {
                        ...
                        musicProviderChanged(.apple_music)
    ...
    private func updateNowPlaying(
        ...
        if lastTrackInfo == currentTrackIdentifier {
            ...
            positionSecondsChanged(positionSeconds)
            durationSecondsChanged(durationSeconds)
            return
        }
        
        lastTrackInfo = currentTrackIdentifier
        
        trackNameChanged(trackName)
        artistNameChanged(artistName)
        albumNameChanged(albumName)
        ...
        if let artworkImage = artworkImage, let imageHash = hashImage(artworkImage) {
            ...
            if lastArtworkIdentifier != currentArtworkIdentifier {
                ...
                DispatchQueue.global(qos: .utility).async {
                    ...
                    DispatchQueue.main.async {
                        dominantColorChanged(dominantColor)
                    }
                }
    ...
    func playPreviousTrack() -> Void {
        DispatchQueue.global(qos: .userInitiated).async {
            if self.is_spotify_playing {
                ...
                """)
            } else if self.is_apple_music_playing {
                _ = self.runAppleScript("""
